...
class VoucherController extends Controller
{
    ...
    public function storeCustomRecord($parameters)
    {
        $voucher = Voucher::create([
            ...
            'expire_date_226'           => $this->request->input('expireDate')? \DateTime::createFromFormat(config('pulsar.datePattern'), $this->request->input('expireDate'))->getTimestamp() : null,
            'expire_date_text_226'      => $this->request->input('expireDate')?  $this->request->input('expireDate') : null,
        ...
        if($this->request->has('bulkCreate') && (int)$this->request->input('bulkCreate') - 1 > 0)
        {
            ...
            VoucherTask::create([
                'user_id_227'               => auth()->guard('pulsar')->user()->id_010,
    ...
    public function updateCustomRecord($parameters)
    {
        Voucher::where('id_226', $parameters['id'])->update([
            ...
            'used_date_226'                 => $this->request->input('usedDate')? \DateTime::createFromFormat(config('pulsar.datePattern'), $this->request->input('usedDate'))->getTimestamp() : null,
            'used_date_text_226'            => $this->request->input('usedDate')?  $this->request->input('usedDate') : null,
            ...
            'paid_226'                      => $this->request->input('payoutDate')? true : false,
